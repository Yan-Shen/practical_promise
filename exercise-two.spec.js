...
describe('exercise two (involving poem two)', function() {
	...
	describe('problemA', function() {
		xit(
			'ignoring errors, logs the first and second stanza in any order, and a done message when both are complete',
			function(done) {
				exercise.problemA();
				setTimeout(function() {
	...
	describe('problemC', function() {
		xit('ignoring errors, logs all stanzas in the correct order, and a done message when all are complete', function(
			done
		) {
			this.timeout(3000);
			exercise.problemC();
	...
	describe('problemD', function() {
		it('logs all stanzas in the correct order; if an error occurs does not read the next file and instead logs the error; always logs done at the end', function(
			done
		) {
			this.timeout(3000);
			exercise.problemD();
